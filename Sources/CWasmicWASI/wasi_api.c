...
// m3_api_wasi.c is distributed under MIT license and the original source
// is here https://github.com/wasm3/wasm3/blob/main/source/m3_api_wasi.c
...
m3ApiRawFunction(m3_wasi_unstable_fd_prestat_get)
{
    ...
    m3ApiWriteMem32(buf+1,  (u32)strlen(preopen[fd].path));
    m3ApiReturn(__WASI_ERRNO_SUCCESS);
}
