...
class WasmExecutorTests: XCTestCase {
    ...
    func testGetExported() throws {
        let exports = try WebAssembly.getExported(wasmBytes: fibWasm)
        let export = try XCTUnwrap(exports.first)
        XCTAssertEqual(export.name, "fib")
        XCTAssertEqual(export.signature, FuncSignature(params: [.i32], results: [.i32]))
    }
}
